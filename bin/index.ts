...
import { TowerflowProjectType } from "../src/interface";
...
commander
  .name(ownPkg.name)
  .description(chalk.cyan("The workflow used by SevenRyze, the COOL guy."))
  .version(ownPkg.version, "-v, --version", `Print the version.`)
  ...
  .action(
    (
      ...
      cmdOptions: {
        bypassNpm: boolean;
        force: boolean;
        template: TowerflowProjectType;
        useCnpm: boolean;
      } = {
        bypassNpm: false,
        force: false,
        template: TowerflowProjectType.WebLib,
        useCnpm: false,
      }
    ) => {
      ...
      const preDefinedPackageJson = Object.assign(
        ...
        [TowerflowProjectType.NodeApp].includes(appType) && {
          bin: {
